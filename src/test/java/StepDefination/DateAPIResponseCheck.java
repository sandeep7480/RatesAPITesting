/********************************************************************************************************
 ...
 Description: Test cases develop to check Date API response
...
public class DateAPIResponseCheck {
	private SoftAssertions softAssertions;
	Response response;
	public static Logger log = Logger.getLogger(DateAPIResponseCheck.class);

	@Given("^Enter below Date API enpoint \"([^\"]*)\"$")
	public void Date_responsecheck(String url) {

		softAssertions = new SoftAssertions();
		response = given().when().get(url);
	}

	@When("^I get Date API response as (\\d+)$")
	public void Date_sucessResponse_as(int arg1) throws Throwable {
		assertEquals("Incorrect status code", arg1, response.getStatusCode());

	}
	

	@Then("^I get Date API JSON response \"([^\"]*)\" and \"([^\"]*)\"$")
	public void DateAPI_jsonResponseCheck(String url, String urldate) {
		...
		log.info("Date API response body is correct");
